...
function Wpaudio (elem) {
	...
	var updateDisplay = function () {
		...
		var button_url = (info.is_playing) ? _wpaudio.url + '/360-button-vis-pause.png' : _wpaudio.url + '/360-button-vis-play.png';
		//var button_url_hover = (info.is_playing) ? _wpaudio.url + '/360-button-vis-pause-light.png' : _wpaudio.url + '/360-button-vis-play-light.png';
		if ( button_url !== jQuery(elem).children('.wpaudio-play').attr('src') ) {
			jQuery(elem).children('.wpaudio-play').attr( 'src', button_url );
			//jQuery(elem).children('.wpaudio-play').hover($(this).attr('src', button_url_hover));
		}
		// Set bar positions
	...
	jQuery(elem).click(function(){
		...
		jQuery(this).blur();
		return false;
	}).prepend('<img class="wpaudio-play" src="' + _wpaudio.url + '/360-button-vis-play.png"/>').wrap('<span class="wpaudio-container"></span>').after(slider);

	// Fix slide width for IE -- in a timeout b/c Chrome shifts otherwise
